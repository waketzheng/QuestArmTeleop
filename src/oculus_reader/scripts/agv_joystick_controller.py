...
class HttpAGVOperator:
    def __init__(self, logger, ip: str, port: int, request_timeout_s: float = 5.0):
        self.logger = logger
...
class AGVJoystickController(Node):
    ...
    def _wait_until_idle(self, timeout_s: float):
        ...
        while time.monotonic() - start_time < timeout_s and not self._stop_event.is_set():
    ...
    def _send_move(self, direction: int, distance_cm: int):
        ...
        est_duration = max(0.15, float(distance_cm) / 100.0 / max(self.move_speed, 0.01))
    ...
    def _execute_control_cycle(self, axis_x: float, axis_y: float):
        ...
        if turning_active:
            ...
            self._send_turn(
                direction=turn_direction, angle_deg=self.arc_turn_angle_deg
            )
